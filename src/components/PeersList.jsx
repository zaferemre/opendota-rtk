...
const PeersList = () => {
    ...
    const { data: peers, isLoading, error } = useGetPeersQuery(playerId, { skip: !playerId });
        ...
        <PeersContainer>
            ...
            {peers.slice(0, 10).map((peer) => (
                <PeerItem key={peer.account_id} >
                    ...
                    <PeerBaseStats>
                        <PeerName href={`/player/${peer.account_id}`}>{peer.personaname || "Unknown"}</PeerName>
                        <PeerGames>{peer.games} games</PeerGames>
                    </PeerBaseStats>
                    <PeerWinLoss>
                        <PeerWinLossPercent>{((peer.win / peer.games) * 100).toFixed(2)}%</PeerWinLossPercent>
                        <PeerWinLossCounts>{peer.win}W - {peer.games - peer.win}L </PeerWinLossCounts>
                    </PeerWinLoss>
                </PeerItem>
            ))}
        </PeersContainer>
