...
const PlayerCard = () => {
  ...
  const { data, error, isLoading } = useGetUserQuery(playerId, { skip: !playerId });

  return (
    <CardContainer>
      {isLoading && <InfoText>Loading...</InfoText>}
      {error && <InfoText style={{ color: "red" }}>Error loading data.</InfoText>}
      {data && (
        <>
          <ProfileHeader>
            <AvatarContainer>
              <Avatar src={data.profile?.avatarfull} alt="Player Avatar" />
              {data.rank_tier && rankTiers[data.rank_tier] && (
                <RankIcon src={`https://cdn.cloudflare.steamstatic.com/apps/dota2/images/dota_react/rank_icons/rank_icon_${rankTiers[data.rank_tier]}.png`} alt="Rank Icon" />
              )}
            </AvatarContainer>
          </ProfileHeader>
          ...
          <ProfileDetails>
            ...
            {data.profile?.profileurl && (
                <SteamCard href={data.profile.profileurl} target="_blank" rel="noreferrer">
