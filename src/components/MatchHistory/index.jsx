...
const MatchItem = styled.div`
  ...
  &:hover {
    background: ${(props) =>
      props.isWin ? "#2A4285" : "#6A314E"}; /* Lighter tint on hover */
  }
`;
...
const WinLossIndicator = styled.div`
  background: ${(props) =>
    props.isWin ? "#3273fa" : "#ff4655"}; /* Blue for win, Red for loss */
  height: 100%;
...
const getGameMode = (id) =>
  capitalizeWords(
    gameModes[id]?.name.replace("game_mode_", "").replace(/_/g, " ") ||
      "Unknown Mode"
  );

const getLobbyType = (id) =>
  capitalizeWords(
    lobbyTypes[id]?.name.replace("lobby_type_", "").replace(/_/g, " ") ||
      "Unknown Lobby"
  );
